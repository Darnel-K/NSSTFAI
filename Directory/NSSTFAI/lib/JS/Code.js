    ...
    if (JSON_OBJ["ChangeableStyles"].hasOwnProperty("CSS")) {
      ...
      for (var Key in JSON_OBJ["ChangeableStyles"]["CSS"]) {
        ...
        for (var LowerKey in JSON_OBJ["ChangeableStyles"]["CSS"][Key][
          ...
          var CSSkey =
            JSON_OBJ["ChangeableStyles"]["CSS"][Key]["Styles"][LowerKey];
          var ID = CSSkey["ID"];
    ...
    if (JSON_OBJ["ChangeableStyles"].hasOwnProperty("JS")) {
      for (var Key in JSON_OBJ["ChangeableStyles"]["JS"]) {
        for (var LowerKey in JSON_OBJ["ChangeableStyles"]["JS"][Key][
          ...
          var CSSkey =
            JSON_OBJ["ChangeableStyles"]["JS"][Key]["Styles"][LowerKey];
          var ID = CSSkey["ID"];
          var Type = CSSkey["Type"];
          var Desc = CSSkey["Desc"];
          ...
          Settings[ID] = Value;
          log(
            "ApplySettings: Setting '" + ID + "' Value Set To '" + Value + "'"
          );
        }
      }
...
function GenBreadCrumb() {
  ...
  for (var i = 2; i < s.length - 1; i++) {
    ...
    log(
      "GenBreadCrumb: Added '" + decodeURIComponent(s[i]) + "' To Breadcrumb"
    );
  }
  breadcrumb += "</ul></div>";
  ...
  if (
    ...
    Settings["SidebarText"] =
      ...
      $("tr.selected")
        ...
        .text();
    log(
      "SetSideBarInfo: Sidebar Information Updated To, Text='" +
        Settings["SidebarText"] +
        "' Image='" +
        Settings["SidebarIconSRC"] +
        "' ImageAlt='" +
        Settings["SidebarIconAlt"] +
        "'"
    );
  }
...
function UpdateAndSaveSettings() {
  if (Settings["Config"].hasOwnProperty("ChangeableStyles")) {
    if (Settings["Config"]["ChangeableStyles"].hasOwnProperty("CSS")) {
      for (var Key in Settings["Config"]["ChangeableStyles"]["CSS"]) {
        var HTML_Element =
          Settings["Config"]["ChangeableStyles"]["CSS"][Key]["Element"];
        for (var LowerKey in Settings["Config"]["ChangeableStyles"]["CSS"][Key][
          "Styles"
        ]) {
          var CSSkey =
            Settings["Config"]["ChangeableStyles"]["CSS"][Key]["Styles"][
              LowerKey
            ];
          var ID = CSSkey["ID"];
          var Type = CSSkey["Type"];
    ...
    if (Settings["Config"]["ChangeableStyles"].hasOwnProperty("JS")) {
      for (var Key in Settings["Config"]["ChangeableStyles"]["JS"]) {
        for (var LowerKey in Settings["Config"]["ChangeableStyles"]["JS"][Key][
          "Styles"
        ]) {
          var CSSkey =
            Settings["Config"]["ChangeableStyles"]["JS"][Key]["Styles"][
              LowerKey
            ];
          var ID = CSSkey["ID"];
          var Value = $("#" + ID).val();
          Settings[ID] = Value;
          Settings["Config"]["ChangeableStyles"]["JS"][Key]["Styles"][LowerKey][
            "Value"
          ] = Value;
          log(
            "UpdateAndSaveSettings: Key '" +
  ...
  localStorage.setItem(
    ...
    encodeURIComponent(JSON.stringify(Settings["Config"]))
  );
...
function SetupSettingsEvents() {
  $("#SettingsButton").on("click", function() {
    if (!Settings["SettingsOpen"]) {
      ...
      $(this)
        .find("img")
        .attr("src", "/NSSTFAI/Icons/Close.svg");
      ...
      $(this)
        .find("img")
        .attr("src", "/NSSTFAI/Icons/Settings.svg");
    ...
    .done(function(json) {
      ...
      if (Settings["Version"] != RemoteVersion["Version"]) {
        if (RemoteVersion["Required"] == true) {
          console.warn(
            "Update: An Important Update Is Available For Download At https://github.com/Darnel-K/Apache-Index-Theme"
          );
        } else {
          console.warn(
            "Update: An Update Is Available For Download At https://github.com/Darnel-K/Apache-Index-Theme"
          );
        }
      } else {
...
function ScrollIfNotVisible(element, parent) {
  try {
    if (
      ...
      $(element).position().top > $(parent).height()
    ) {
      $(parent).scrollTop(
